...
class FCareTaker {
public:
    FCareTaker(FighterMemento* ptmpfm)
        : m_pfm(ptmpfm)
    {
    }
...
public:
    ...
    {
        try {
            return m_pfmContainer.at(index);
...
int main(void)
{
    ...
    // (2) 保存当前游戏进度
    FCareTaker* pfct = new FCareTaker(p_fighter->createMemento());
    ...
    // (5) 恢复玩家主角对象数据
    p_fighter->restoreMemento(pfct->getMemento());
    cout << "---------- 恢复检查点 ---------- " << endl;
    p_fighter->displayInfo();

    delete pfct->getMemento();
    delete pfct;
    delete p_fighter;
